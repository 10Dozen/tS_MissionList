var missionsInfo = [
	{
		...
		, "tags": ["Inf", "Inf-Only", "Ligth Vehicles", "Ligth Vehicles", "Ligth Vehicles", "Ligth Vehicles"]
	...
	, {
		"id": 5
		, "filename": "CO35-Manic-Mansion-1A.Takistan"
		, "title": "CO35 Manic Mansion (1A)"
		, "player_count": 35
		, "terrain": "Takistan"
		, "tags": []
		, "overview": "Takistan Army patrols wastelands"
...
var GridViewClass = function () {
	...
	this.refreshGrid = function(model) {
		...
		while (model.hasNext()) {
			let info = model.next();
			let tags = "";
			(info.tags).forEach(function(el) { tags = tags.concat(`${el}<br/>`) });
			...
			$(this.$grid).append(`<tr class="grid-line" mission-id="${info.id}">`
				...
				+ `<td class="td-overview">${tags}</td>`
				+ `<td class="td-center"><div class="btn-see-more">Details</div></td>`
			+ "</tr>");
		}
...
var GridControllerClass = function () {
	...
	this.$btn_seeMore = "#grid tr td div[class='btn-see-more']";
	...
	this.initEvents = function () {
		...
		$(this.$btn_seeMore).on("click", this, function (event) {
			let model = event.data.model;
			let missionId = parseInt( $(this).parent().parent().attr("mission-id") );
...
$( document ).ready(function () {
	/*
		[x] Details modal window event - open/close
		[x] Details modal window - show info

		[x] Filtering by mission name
		[x] Filtering by player count
		[x] Filtering by terrain

		[x] Fix filtering and sorting interference 
		[ ] Add tS styles
	*/
